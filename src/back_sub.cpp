        ...
        for (int j = 0; j < img_cols; j++)
        {
            ...
            if (should_threshold)
            {
                ...
                else if (del_b < 0)
                {
                    ...
                    continue; // Ignores static parts of the image -> del_b == 0;
            }

            out_img_unthresh.at<uchar>(i, j) = static_cast<uchar>(std::abs(del_b));
            if (del_b > 0)
                out_img_unthresh_bright.at<uchar>(i, j) = static_cast<uchar>(del_b);
        }
    }
...
bool morph_min_max(const Bg_sub::pt_t &pt, const cv::Mat &k, const cv::Mat &padded_img, const uchar comp_val)
{
    ...
    for (int i = 0; i < k_size; i++)
    {
        ...
        {
            if (k.at<uchar>(i, j) == 1 && padded_img.at<uchar>(pt.x + offset + i, pt.y + offset + j) == comp_val)
                return true;
        }
    }
    return false;
